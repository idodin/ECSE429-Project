Feature: Query remaining tasks
  ...
  Scenario: The user successfuly queries all imcomplete task in a class project (Normal Flow)
  ...
  Scenario: The user successfuly queries all imcomplete task in a class projects with all done task (Alternate Flow)
  Given Categories exist for the following priority levels:
  | priority |
  | HIGH     |
  | MEDIUM   |
  ...
  Scenario Outline: The user queries tasks from project that doesn't exist (Error Flow)
